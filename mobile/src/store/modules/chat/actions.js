...
export function socketMessage(message){
  return {type: ADD_SOCKET_MESSAGE, message, receivedAt: Date.now()}
}
...
export function readMessagesSuccess(session){
  return { type: READ_MESSAGES_SUCCESS, session, receivedAt: Date.now()}
}
...
export function sendReadMessages(sessionId) {
  ...
  return function(dispatch) {
    ...
    .then(([response, jsonBody]) => {
      ...
      if (response.status == 200) {
        dispatch(readMessagesSuccess(jsonBody))
      } else {
