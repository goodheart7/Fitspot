...
class ChooseTrainer extends React.Component{
    ...
    selectTrainer(trainer){
      var trainerName = trainer.id  < 0 ? "Fitspot Choose" : trainer.publicId;
      events.bookingChoseTrainer(this.props.bookingState.bookingType, trainerName);
        ...
        <TouchableHighlight onPress={() => this.selectTrainer(rowData)} style ={styles.rowStyle} underlayColor={'rgba(0,0,0,0)'}>
          <View style={{alignSelf:'center',justifyContent:'center',alignItems:'center', marginBottom:6}}>
            ...
            <View>
              ...
              { rowData.avatar ? <Image source={{uri: rowData.avatar.url}} style={{width:72,height:72 ,borderRadius:36}}/>
    ...
    onSearchChange(text) {
      ...
      this.props.trainers.forEach(function(item) {
        if (item.publicId.toLowerCase().indexOf(filter) !== -1 || item.id < 0) {
          newFilter.push(item)
        }
      });
      this.setState({
          ...
          <View style={{ flex: 1,
            ...
            backgroundColor: 'green'}}>
                  <TextInput
                    ...
                    onEndEditing={(text) => {
                      events.bookingTrainerSearch(this.props.bookingState.bookingType, text);
                    }}
                  />
          </View>
          <View style={{flex: 16}}>
