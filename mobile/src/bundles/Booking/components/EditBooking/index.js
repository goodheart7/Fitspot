...
class EditBooking extends React.Component {
  ...
  chat(workoutItem){
    var chatSession = this.props.chatSessions.filter( chatSession => {
      if(chatSession.customer.id === workoutItem.createdByUserId &&
  ...
  render() {
    ...
    if(this.props.userType !== CONSTS.USER_TYPE.TRAINER){
      nameToDisplay = !workoutItem.trainer ? 'Fitspot Choose' : workoutItem.trainer.firstName + ' ' + workoutItem.trainer.lastName
    }
