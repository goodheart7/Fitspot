  ...
  async changeCard(){
    ...
    try {
      ...
      if(token[0] === 'error'){
        ...
        events.bookingEnteredCard(this.props.bookingState.bookingType);
  ...
  async buyItem(){
    ...
    if(appSettings.braintreeClientToken === null){
      ...
      try {
        ...
        ApiUtils.post('customers/purchase-plan',{
          planId: rowData.id,
          paymentNonce: token[0],
          isEnterprise: false,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(nextProps.isAdding){
      ...
      if(nextProps.error.length > 0){
        ...
        this.setState({btnSubmitState: 'idle'});
        var bookingType = (this.props.bookingState.bookingType == CONSTS.BOOKING_TYPE.BY_ACTIVITY ? 'BBA' : 'BBT');
        events.bookingDone(this.props.bookingState.bookingType);
            ...
            <View style={{width:Dimensions.get('window').width,backgroundColor:'#F2F2F2',borderBottomColor:'#F2F2F2', borderBottomWidth:1,flex:1,alignSelf: 'stretch',marginRight:22}}>
              <View style={{flexDirection:'row',justifyContent:'space-between',flex:1,marginTop: 12,marginBottom: 12,}}>
                ...
                <View style={{justifyContent:'center'}}>
                  ...
                  <Text style={{marginRight:16,fontFamily:FONT_DAYTONA_LIGHT,textAlign:'center', fontSize:20,color:'#4B4B4C'}}>${this.state.bookingState.customerPrice}</Text>
                </View>
              </View>
            </View>
