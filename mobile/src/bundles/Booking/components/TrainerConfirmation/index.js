...
class BookingTrainerConfirmation extends React.Component {
  ...
  chat(workoutItem){
    var chatSession = this.props.chatSessions.filter( chatSession => {
      if(chatSession.customer.id === workoutItem.createdByUser.id &&
        chatSession.trainer.id === workoutItem.trainerId)
        return chatSession;
    })

    if(chatSession.length > 0){
      Actions.chatScene({sessionId: chatSession[0].sessionId, name: workoutItem.createdByUser.firstName + ' ' + workoutItem.createdByUser.lastName})
    }else{
  ...
  fireAction(action){
    var eventProps = {
      trainer : this.props.workoutItem.trainer.publicId,
      user : this.props.workoutItem.createdByUser.publicId,
      id : this.props.workoutItem.id
    };
    if(action === 'decline'){
      events.track('Session Declined', eventProps);
      // TODO???
    }else if(action==='confirm'){
      events.track('Session Confirmed', eventProps);
      ...
      Actions.mainAppModal(
        {
          ...
          showSubDetails: false,
          onOkay: () => {
            events.track('Session Cancelled', eventProps);
            this.props.cancelWorkout(this.props.workoutItem.id);
          },
          okayButtonText: 'OK',
  ...
  changeDate(date){
    ...
    var dateString = moment(date).format('dddd, MMMM Do h:mm A')
  ...
  render() {
    ...
    var nameToDisplay =  workoutItem.createdByUser.firstName + ' ' + workoutItem.createdByUser.lastName
      ...
      return (
        <View style={{ flex:1,marginTop:65,marginBottom:55,borderTopColor:'#E6E6E6',borderTopWidth:1}}>
          ...
          <Button buttonStyle={[styles.bookButtonStyle,{borderBottomWidth:.5,borderColor:'#CaCaCa'}, typeof workoutItem.trainer.id === 'undefined' ? {backgroundColor:'#CACACA'}:null]} buttonTextStyle={styles.bookButtonTextStyle} onPress={() => this.chat(workoutItem)}>
            {typeof workoutItem.trainer.id === 'undefined' ? 'No trainer assigned, chat unavailable.'  : 'Chat with '+ workoutItem.createdByUser.firstName}
          </Button>
          { workoutItem.status === CONSTS.WORKOUT_STATUS.PENDING && this.props.userType === CONSTS.USER_TYPE.TRAINER  ?
            <View style={{flexDirection:'row'}}>
              <Button buttonStyle={[styles.confirmCancelButton,{backgroundColor:'red'}]}
                      buttonTextStyle={styles.bookButtonTextStyle}
                      onPress={() => this.fireAction('cancel')}
              >
                Decline Workout
              </Button>
              <Button buttonStyle={[styles.confirmCancelButton,{borderLeftColor:'#CACACA',borderLeftWidth:.5}]}
                      buttonTextStyle={styles.bookButtonTextStyle}
                      onPress={() => this.fireAction('confirm')}
              >
                Confirm Workout
              </Button>
            </View>
