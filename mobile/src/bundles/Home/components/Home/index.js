...
class Home extends React.Component {
  ...
  getNextWorkout(){
    var nextWorkout = sortBy(this.props.workoutInfo.workoutItems, [function(w) {
      return moment(w.dtStart ? w.dtStart: w.date).unix()
    }])[0];
    return nextWorkout;
  }
  renderTopView(){
  ...
  renderCustomer(){
    ...
    return (
      <Image source={require('../../assets/newHeader.jpg')} style={styles.clientContainer}>
        ...
        { this.renderTopView() }
        <Button buttonStyle={styles.bookActivityButton}
                buttonTextStyle={styles.bookActivityButtonText}
                onPress={() => this.setBookingType(CONSTS.BOOKING_TYPE.BY_ACTIVITY)}
                buttonName={'Book By Activity'}
        >
          Book an Activity
        </Button>
        ...
        <Button buttonStyle={styles.registerButton}
                ...
                onPress={() => this.setBookingType(CONSTS.BOOKING_TYPE.BY_TRAINER)}
                buttonName={'Book By Trainer'}
        >
          Browse Trainers
        </Button>
        </View>
    ...
    return (
      <TouchableHighlight onPress={() => Actions.confirmTrainerWorkoutHome({workoutItem: rowData})}>
        <View style={{backgroundColor:'black', flexDirection:'row',borderTopColor:'#4a4a4a',borderTopWidth:1,alignItems:'center'}}>
          { rowData.createdByUser.avatar === null ?
            <Image style={{
              borderRadius: 25,
              width:50,
              height: 50,
          ...
          <Image style={{
            ...
            marginLeft: 16,
            // position:'absolute'
          }} source={{uri: rowData.createdByUser.avatar.url}} />
        }
        <View>
          ...
          <Text style={{fontFamily:FONT_DAYTONA_SEMIBOLD,fontSize:12,letterSpacing:-.1,color:'white'}}>{moment(rowData.dtStart).format('hmmA ddd')} with {rowData.createdByUser.firstName} {rowData.createdByUser.lastName}</Text>
    ...
    return (
      <View style={styles.clientContainer}>
          ...
          <View style={{paddingBottom:20,backgroundColor:'rgba(255,255,255,.2)',flexDirection:'row',justifyContent:'space-around', width:Dimensions.get('window').width, marginBottom: 20}}>
            <View>
              <Text style={{color:'white',textAlign:'center', fontFamily: FONT_DAYTONA_LIGHT, fontSize: 24}}>{this.props.workoutInfo.paymentHistory.numWorkouts || 0}</Text>
              <Text style={{color:'white',textAlign:'center', fontFamily: 'System', fontWeight:'bold',fontSize: 8}}>WORKOUTS</Text>
            </View>
            <View>
              <Text style={{color:'white',textAlign:'center',fontFamily: FONT_DAYTONA_LIGHT, fontSize: 24}}>{this.props.workoutInfo.paymentHistory.totalEarned || 0}</Text>
              <Text style={{color:'white',textAlign:'center', fontFamily: 'System', fontWeight:'bold',fontSize: 8}}>EARNED</Text>
            </View>
            <View>
              <Text style={{color:'white',textAlign:'center',fontFamily: FONT_DAYTONA_LIGHT, fontSize: 24}}>{this.props.workoutInfo.paymentHistory.numClients || 0}</Text>
