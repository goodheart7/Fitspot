...
class BraintreeDropIn extends React.Component {
  ...
  addPaymentMethod(event) {
    ...
    this.tokenize().then((newcard) => {
      ...
      events.track("Added New Credit Card");
    }).catch((err) => {
        this.setState({
            cardsLoaded: true,
        });
      ...
      <div className="braintree-dropin">
        ...
        <div className="list-group" style={{display: isLoaded ? 'block': 'none' }}>
          ...
          <div className="list-group-item" style={cardStyle}>
            ...
            <button onClick={this.addPaymentMethod} className="btn btn-info btn-lg btn-block">Add Card</button>
          </div>
        </div>
      </div>
    );
