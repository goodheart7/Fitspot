...
class BookingHeaderContainer extends Component {
  ...
  getHeaderTextElements() {
    ...
    var dateTime= bookingState.chosenDate ? bookingState.chosenDate.local().format("MMM DD h:mm a") : '';
  ...
  renderHeaderText() {
    ...
    return (
      <span>
        ...
        <span className={"steps-text hidden-xs hidden-sm hidden-md pull-right " + (bookingState.currentStep === bookingState.totalSteps && 'booking-step-7')}>
            {bookingState.currentStep !== bookingState.totalSteps && <small>STEP</small>}
            {bookingState.currentStep !== bookingState.totalSteps ? `${bookingState.currentStep}/${bookingState.totalSteps}` : <i className="fa fa-check" aria-hidden="true" />}
        </span>
        <span className="logo-devider hidden-xs hidden-sm hidden-md pull-right" />
      </span>
    )
  }
  _getActivity() {
    ...
    return(
      <div className="btn-group">
          <button type="button" className="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <span className="mobile-steps"><i className="fa fa-check" aria-hidden="true" /></span>
