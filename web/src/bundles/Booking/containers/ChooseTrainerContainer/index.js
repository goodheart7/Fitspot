...
import { selectTrainer, fetchTrainers, searchTrainerAvailabilities, addSubscriptionOption, increaseCurrentStep, setCurrentStep, CHOOSE_TRAINER_STEP } from '@store/modules/booking/actions';
...
class ChooseTrainerContainer extends Component {
  ...
  fetchTrainers() {
    ...
    if(this.props.bookingState.bookingType == CONSTS.BOOKING_TYPE.BY_ACTIVITY) {
      this.props.searchTrainerAvailabilities(bookingState.chosenLocation.lat,
        bookingState.chosenLocation.lon, 20, bookingState.chosenActivity.id,
      ...
      this.props.searchTrainerAvailabilities(33.8101512, -84.4225184, 20,
        undefined,
        nearestBookingTime().format());
    }

  }

  componentDidMount() {
    this.props.setCurrentStep(CHOOSE_TRAINER_STEP);
    this.fetchTrainers();
  }
  ...
  onChooseTrainer(trainer) {
    ...
    events.bookingChoseTrainer(this.props.bookingState.bookingType, trainer.id  < 0 ? "Fitspot Choose" : trainer.publicId);
    if(this.props.bookingState.bookingType == CONSTS.BOOKING_TYPE.BY_ACTIVITY) {
